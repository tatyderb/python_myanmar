...
## for - перебрать последовательность элементов

Цикл можно написать с помощью `while` или `for`.

**for** перебирает по элементу заданную последовательность. Напечатаем фразу "цвет настроения ..." для разных цветов:
...
`_` - такая же переменная, как и остальные. Можно написать `_ = _ + 1`, но мы подчеркиваем именем, что переменная в цикле использоваться не будет.

## QUIZ range(6)

Какую последовательность чисел даст `range(6)`?
...
Какую последовательность чисел даст `range(1, 5)`?
...
Какую последовательность чисел даст `range(4, 17, 3)`?
