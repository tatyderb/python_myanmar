...
## TASKINLINE Bloop_path3 Сколько прошел робот за 20 шагов?

Робот шагает. Первый шаг длиной L метров, каждый следующий на k **меньше**.

Если шаг робота ≤ 0, то робот дальше не идет (робот топает на месте).

Программа, которая вычисляет длину пути робота за 7 шагов:

```python
# возвращает длину следующего шага
def next_step(step, k):
    step = step - k
    # робот не идет назад, а топчется на месте
    if step < 0:
        step = 0
...
# Дано
L = 4           # длина первого шага
k = 2           # на сколько меньше будет следующий шаг

# готовимся шагать, до цикла 1 раз
path = 0        # еще ничего не прошли
step = L        # первый шаг длиной L
i = 0           # еще не закончился первый шаг

# цикл
while i < 7:    
    path = path + step
    i = i + 1                   # шаг закончился
    print(f'шаг {i}: прошли {step} м, всего {path} м')
    step = next_step(step, k)   # готовимся к следующему шагу

# ответ ПОСЛЕ цикла
print(f'Всего прошли {path} м')
```

* Переписать и отладить программу для **20 шагов**.
* Программа должна печатать ТОЛЬКО ПОСЛЕДНИЙ path. Убрать ненужные print.
...
## TASKINLINE Bloop_path4 Сколько прошел робот до остановки?

Робот шагает. Первый шаг длиной L метров, каждый следующий на k **меньше**.

Если шаг робота ≤ 0, то робот дальше не идет (робот останавливается).

Программа, которая вычисляет количество шагов и длину пути робота до его остановки.
...
# цикл
while step > 0:                 # пока шагаем вперед   
    path = path + step          # робот сделал шаг
    print(f'прошли {step} м, всего {path} м')
    step = next_step(step, k)   # готовимся к следующему шагу
...
Написать программу, которая вычисляет длину пути и количество шагов, которые сделал робот.

Программа должна печатать ТОЛЬКО **path** и **i**.
...
## TASKINLINE Bloop_path5 Сколько не дошел робот?

* Расстояние от старта до финиша равно **dist**.
* Робот должен пройти сколько может.
