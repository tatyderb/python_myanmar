...
Плохо: много раз вычислять $\sqrt{b^2-4ac}$

Хорошо: один раз вычислить, **сохранить результат в памяти компьютера** и дальше использовать сохранённые данные.
...
Результат вычисления можно тоже записать в переменную, например, переменную `d` (от слова "детерминант"). Посмотрим, чему равно значение переменной после присвоения:
